...
(function (VMail) {
    ...
    (function (App) {
        ...
        var showTopContacts = function (topN) {
            ...
            if (!App.isWithinRange) {
                var topContacts = App.db.getTopContacts(topN, undefined, undefined);
            } else {
                var topContacts = App.db.getTopContacts(topN, start, end);
            }
        ...
        var showContactDetails = function (contact, start, end) {
            ...
            var contactDetails = App.db.getContactDetails(start, end);
            ...
            $(container.append("div").html(contact.name).attr("class", "person_name").attr('title', '').node()).tooltip({ content: content });

            container.append("div").html('<b>First email:</b> ' + longAgo(new Date(), App.db.contactDetails[contact.id].firstEmail) + " ago").attr("id", "firstemail");
            container.append("hr");
            container.append("div").html('<b>Last email:</b> ' + longAgo(new Date(), App.db.contactDetails[contact.id].lastEmail) + " ago");
            ...
            var timestamps = App.db.getEmailDatesByContact(contact);
            ...
            var introductions = App.db.getIntroductions(contact);
        ...
        var initSearchBox = function () {
            ...
            $("#searchContacts").autocomplete({
                ...
                source: function (request, response) {
                    ...
                    var results = [];
                    for (var id in App.db.contacts) {
                        var contact = App.db.contacts[id];
                    ...
                    results.sort(function (a, b) {
                        ...
                        if (idx1 === 0) {
                            if (idx2 === 0) {
                                return getScore(App.db.contacts[b.value]) - getScore(App.db.contacts[a.value]);
                            } else {
                                return -1;
                            }
                        } else {
        ...
        var initTimeSlider = function () {
            $("#slider-ytd").click(function () {
                var c = App.db.emails[App.db.emails.length - 1].timestamp * 1000;
                end = new Date(c);
            ...
            $("#slider-all").click(function () {
                var a = App.db.emails[0].timestamp * 1000;
        ...
        var initBasicInfo = function (aliases, userinfo) {
            ...
            $('#ncontacts').html(numParser(App.db.nCollaborators) + ' collaborators');
            $('#totalemails').html(numParser(App.db.emails.length) + ' emails');
            /*
        ...
        var initHistograms = function () {
            var timestampsSent = App.db.getEmailDatesSent();
            var timestampsRcv = App.db.getEmailDatesRcv();
            var a = +new Date(App.db.emails[0].timestamp * 1000);
            ...
            var timestampsNewContacts = App.db.getTimestampsNewContacts();
        ...
        var sendStatsToServer = function () {
            var data = {};
            data['ncollaborators'] = App.db.nCollaborators;
            data['nsent'] = App.db.nSent;
            data['nrcv'] = App.db.nRcv;

            // reply times
            var myReplyTimes = App.db.myReplyTimes;
            var othersReplyTimes = App.db.othersReplyTimes;
        ...
        var showData = function () {
            ...
            start = new Date(App.db.emails[0].timestamp * 1000);
            end = new Date();

            //populate the left column with some basic info and aggregate statistics
            initBasicInfo(App.db.aliases, App.userinfo);
            ...
            d3.range(App.version + 1).forEach(function (i) {
                $.ajax({
                    ...
                    success: function (emails) {
                        alert("In line 978 of app.js loading emails")
                        console.log(emails)
